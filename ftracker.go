...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// Средняя скорость в м/c
	meanSpeedMpS := (meanSpeed(action, duration) * kmhInMsec)
	heightM := height / cmInM
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMpS, 2)/heightM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
